...
function drawInteractiveInterface(node, ctx) {
    ...
    if (!hasModelData) {
        ...
        ctx.font = "14px Arial";
        ctx.fillText("等待模型数据...", 10, 140);

        ctx.fillStyle = "#888888";
        ctx.font = "12px Arial";
        ctx.fillText("请先运行工作流以获取模型列表", 10, 160);

        ctx.fillText("此节点将在工作流运行时显示交互式模型选择界面", 10, 180);
    }
    ...
    if (hasModelData) {
        ...
        ctx.font = "14px Arial";
        ctx.fillText(`发现 ${models.length} 个未使用模型`, 10, 140);

        // 绘制操作信息 - 适应300px宽度
        ctx.fillStyle = "#cccccc";
        ctx.font = "12px Arial";
        ctx.fillText(`操作模式: ${interactiveModelCleanerState.actionMode}`, 10, 160);

        // 备份路径显示，300px宽度需要截断更多内容
        let backupText = `备份文件夹: ${interactiveModelCleanerState.backupFolder}`;
        ...
        ctx.fillText("点击模型名称选择/取消选择，然后点击确认", 10, 200);
    ...
    if (hasModelData) {
        ...
        for (let i = 0; i < visibleModels; i++) {
            ...
            if (modelName.length > 30) {  // 调整截断长度以适应300px宽度
            ...
            ctx.fillStyle = "#cccccc";
            ctx.font = "9px Arial";
            ctx.fillText(`${model.size_formatted} | 未使用置信度: ${confidence}%`, 28, y + 25);  // 从8改为28
        ...
        if (selectedCount > 0) {
            ctx.fillStyle = "#ffff88";
            ctx.font = "11px Arial";
            ctx.fillText(`已选择 ${selectedCount} 个模型`, 10, node.size[1] - 90);  // 从-20改为-90，避免与按钮重叠
        }
    ...
    ctx.fillStyle = "#ffffff";
    ctx.font = "13px Arial";
    ctx.fillText("取消", cancelX + 25, buttonY + 20);  // 调整文字位置以适应80px按钮宽度
    ...
    ctx.fillStyle = "#ffffff";
    ctx.font = "13px Arial";
    const confirmText = hasModelData ? "确认" : "继续";
    ctx.fillText(confirmText, confirmX + 25, buttonY + 20);  // 调整文字位置以适应80px按钮宽度
